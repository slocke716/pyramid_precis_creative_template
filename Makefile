CODE_PATHS := pyramid_precis_creative_template
...
CI_REGISTRY_IMAGE			?= ${CI_REGISTRY}/stevenlocke/pyramid_precis_creative_template
...
.build_${PYTHON_VERSION}: Dockerfile pyramid_precis_creative_template clean__py
	${_CMD_DOCKER_BUILD} \
...
test:	## run pytest on mounted-in tests (change tests, no rebuild!)
	...
	docker run \
			--platform linux/amd64 \
			--volume $(shell pwd)/bin/:/pyramid_precis_creative_template/bin/ \
			--volume $(shell pwd):/pyramid_precis_creative_template:rw \
			${DOCKER_IMAGE_TAG} \
			pytest --cov-report xml:/pyramid_precis_creative_template/bin/coverage.xml tests/
...
local-publish:
	echo "Local poetry publishing. You need to export the envs first"
